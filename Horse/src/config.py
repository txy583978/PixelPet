...
class Config:
    # ==================== Sprite 配置 ====================
    sprite_path: str = "assets/horse.png"
    frame_width: int = 32
    frame_height: int = 32
    ...
    flip_h_directions: set = field(default_factory=lambda: {1})  # 朝左水平翻转
    ...
    sound_files: List[str] = field(default_factory=lambda: ["hoof.wav"])
    walk_sound_enabled: bool = True
    walk_sound_interval: float = 0.45
    walk_sound_only: bool = True
    sound_min_interval: float = 20.0
    sound_max_interval: float = 40.0
    sound_volume: float = 0.5

    # ==================== 交互配置 ====================
    quit_alpha_increment: float = 0.0033
    jump_velocity: float = 350.0   # 马跳得高一点
    gravity: float = 800.0
