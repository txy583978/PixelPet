...
class Config:
    # ==================== Sprite 配置 ====================
    sprite_path: str = "assets/ostrich.png"
    frame_width: int = 32
    frame_height: int = 32
    ...
    # 标准行顺序：row0=正面(下), row1=朝右, row2=背面(上), row3=朝左, row4=吃/睡
    # 羊式逻辑：只用朝右帧(row1)，左方向翻转
    frame_map: Dict[str, Dict[int, int]] = field(default_factory=lambda: {
        "walk":  {0: 1, 1: 1, 2: 2, 3: 3},
        "idle":  {0: 1, 1: 1, 2: 2, 3: 3},
        "eat":   {0: 4, 1: 4, 2: 4, 3: 4},
    ...
    sound_burst_interval: float = 0.8
